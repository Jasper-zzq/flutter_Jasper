...
class SmartSelectState<T> {
  ...
  SmartSelectOption<T> get valueObject {
    return options.firstWhere(
        (SmartSelectOption<T> item) => item.value == value,
        orElse: () => null);
  }
  ...
  List<SmartSelectOption<T>> get valuesObject {
    return options
        .where((SmartSelectOption<T> item) =>
  ...
  String get valueDisplay {
    return isMultiChoice == true
        ? valuesTitle != null ? valuesTitle.join(', ') : placeholder
        : valueTitle != null ? valueTitle : placeholder;
  }
}
